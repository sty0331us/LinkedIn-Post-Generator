...
이 모듈은 Google Gemini API를 사용하도록 설계되었습니다.
...
def ensure_gemini_client():
    if genai is None:
        raise ImportError(
            "Gemini client is not installed. Install it with `pip install google-generativeai`."
        )
...
def generate_post(client, persona: str, input_text: str) -> str:
    """Generate one LinkedIn post using Gemini."""
    prompt = (
    ...
    raise RuntimeError(
        "Unsupported Gemini client interface. Please verify the google.generativeai package version."
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a LinkedIn post dataset using Gemini."
    )
    parser.add_argument(
        "--num-samples",
        type=int,
